const HarleyFlix = {
  ...
  handleResponsiveSearch() {
    const handleView = () => {
      if (this.isMobile()) {
        if (!this.els.searchOverlay.contains(this.els.searchContainer)) {
          this.els.searchOverlay.prepend(this.els.searchContainer);
        }
      } else {
        if (!this.els.navSearchContainer.contains(this.els.searchContainer)) {
          this.els.navSearchContainer.appendChild(this.els.searchContainer);
        }
      }
    };
    handleView();
    window.addEventListener('resize', this.debounce(handleView, 100));
  },
  addEventListeners() {
    document.getElementById('watched-list-btn').addEventListener('click', () => {
      this.showWatchedModal();
      document.getElementById('watched-modal').style.display = 'flex';
    });
    document.getElementById('watched-modal-close').addEventListener('click', () => {
      document.getElementById('watched-modal').style.display = 'none';
    });
    document.getElementById('watched-clear-btn').addEventListener('click', () => this.clearWatchedList());
    this.els.modalCloseBtn.addEventListener("click", () => this.closeModal());
    this.els.searchCloseBtn.addEventListener('click', () => this.clearSearch());
    this.els.navSearchTrigger.addEventListener('click', () => {
    ...
    document.addEventListener('click', (e) => {
        if (!this.els.searchContainer.contains(e.target)) {
  ...
  clearSearch() {
    ...
    this.els.searchOverlayResults.innerHTML = '';
  },
  isMobile() {
  ...
  async handleSearch() {
    ...
    const resultsContainer = isMobile ? this.els.searchOverlayResults : this.els.searchResultsDropdown;
  ...
  addToWatched(details) {
    ...
    const entry = {
      id: details.id,
      media_type: details.media_type || (details.first_air_date ? 'tv' : 'movie'),
      title: details.title || details.name,
      poster_path: details.poster_path,
      watched_at: Date.now()
    };
    this.watchedList = this.watchedList.filter(item => !(item.id === entry.id && item.media_type === entry.media_type));
    this.watchedList.unshift(entry);
    localStorage.setItem('watchedList', JSON.stringify(this.watchedList));
  },
  loadWatchedList() {
    const raw = localStorage.getItem('watchedList');
    this.watchedList = raw ? JSON.parse(raw) : [];
  },
  showWatchedModal() {
    this.loadWatchedList();
    const container = document.getElementById('watched-list-container');
    const emptyMsg = document.getElementById('watched-empty-msg');
    container.innerHTML = '';
    ...
    this.watchedList.forEach(item => {
      ...
      div.onclick = () => this.openWatchedItem(item);
      const img = document.createElement('img');
      img.src = item.poster_path ? `https://image.tmdb.org/t/p/w185${item.poster_path}` : 'https://placehold.co/100x150/333/fff?text=No+Image';
      const label = document.createElement('div');
      label.className = 'watched-item-title';
  ...
  async openWatchedItem(item) {
    const details = await this.fetchMediaDetails(item.id, item.media_type);
    if (details) this.openModal(details);
    document.getElementById('watched-modal').style.display = 'none';
  },
  clearWatchedList() {
    if (window.confirm('Clear watched list?')) {
